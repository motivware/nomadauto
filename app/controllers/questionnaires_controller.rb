class QuestionnairesController < ApplicationController
  ...
  def create
    @project = Project.find(params[:project_id])
    @questionnaire = @project.questionnaires.where(project_id: @project, id: @questionnaire)

    respond_to do |format|
        if @questionnaire.create(questionnaire_params)
          puts questionnaire_params
          flash[:success] = 'Record Saved.'
          format.html { redirect_to project_questionnaires_url(@project), notice: "Questionnaire was successfully created." }
